...
# Label mapping: assumed cluster 1 = med, 2 = high, 3 = low (based on sorted centers)
cluster_to_label <- c("med", "high", "low")

# Apply labels and factor levels for color consistency
county_risk_df <- county_risk_df %>%
  mutate(risk_level = factor(cluster_to_label[as.numeric(cluster)],
...
risk_colors <- c("low" = "#56B1F7", "med" = "#009E73", "high" = "#D55E00")

# County map plot
ggplot(counties_polygon_TX_clust, aes(long, lat)) + 
  geom_polygon(aes(group = group, fill = risk_level), color = "white") +
  scale_fill_manual(values = risk_colors) +
  coord_quickmap() + 
  labs(title = "Texas Map with Age Clustered Counties", fill = "Risk Level")
